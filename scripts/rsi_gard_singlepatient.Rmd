...
dir <- ("~/Desktop/rsi_analysis/results/luminal/")
files <- file.path(dir, rownames(samples), "t_data.ctab") 
...
## StringTie input
```{r}
tmp <- read_tsv("~/Desktop/rsi_analysis/data/single_hisat/SRR6671104/t_data.ctab")
tx2gene <- tmp[, c("t_name", "gene_name")] #perform this for all files
```
...
```{r}
rsi.all.stringtie <- calc_RSI_GARD(filteredExpr) #calculate RSI with normalized counts
rsi.all.stringtie
```
...
rsi.all.condition <- merge(rsi.all.stringtie, samples, by=0)
...
rsi.all.stringtie <- rownames_to_column(rsi.all.stringtie, "sample")
```
...
#
g2 <- ggplot(rsi.all.stringtie, aes(x=sample, y=RSI, color = sample)) +
  geom_point() +
